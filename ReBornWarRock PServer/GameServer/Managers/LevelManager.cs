...
namespace ReBornWarRock_PServer.GameServer.Managers
{
    class LevelCalculator
    {
        ...
        public static void SetRates(int Rate)
        {
            int i = 0;
            long _TEMP;
            int level;
            while (i < EXPTable.Length - 1)
            {
                _TEMP = EXPTable[i] / Rate;
                EXPTable[i] = _TEMP;
                level = i + 1;
                //Log.AppendText("Exp for Level [" + level + "] = " + EXPTable[i]);
                Log.WriteFile("Exp for Level [" + level + "] = " + EXPTable[i]);
                i++;
            }
        }
